...
After installing CodePush CLI globally, it will be available under `revampush`.
...
```
revampush register <optional: server-url>
```
...
```shell
revampush login <optional: server-url>
```
...
```shell
revampush session ls
revampush session rm <machineName>
```
...
```shell
revampush access-key add "VSTS Integration"
```
...
```shell
revampush login --accessKey <accessKey>
```
...
```shell
revampush access-key patch <accessKeyName> --name "new name" --ttl 10d
```
...
```
revampush app add MyApp-Android
revampush app add MyApp-iOS
```
...
```
revampush app rename <appName> <newAppName>
```
...
```shell
revampush collaborator add <appName> <collaboratorEmail>
```
...
```shell
revampush collaborator rm <appName> <collaboratorEmail>
```
...
```shell
revampush collaborator ls <appName>
```
...
```shell
revampush app transfer <appName> <newOwnerEmail>
```

_NOTE: Just like with the `revampush collaborator add` command, this expects that the new owner has already registered with CodePush using the specified e-mail address._

Once confirmed, the specified developer becomes the app's owner and immediately receives the permissions associated with that role. Besides the transfer of ownership, nothing else about the app is modified (e.g. deployments, release history, collaborators). This means that you will still be a collaborator of the app, and therefore, if you want to remove yourself, you simply need to run the `revampush collaborator rm` command after successfully transferring ownership.
...
```
revampush deployment add <appName> <deploymentName>
```

If you want to re-use an existing deployment key, you can do this with:

```
revampush deployment add <appName> <deploymentName> -k <existing-deployment-key>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
revampush deployment rm <appName> <deploymentName>
revampush deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
```
revampush deployment ls <appName> [--displayKeys|-k]
```
...
2. [React Native](#releasing-updates-react-native) - Performs the same functionality as the general release command, but also handles the task of generating the updated app contents for you (JS bundle and assets), instead of requiring you to run both `react-native bundle` and then `revampush release`.
...
```
revampush release <appName> <updateContents> <targetBinaryVersion>
...
This specifies the name of the CodePush app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `revampush app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `revampush app ls` command to see your list of apps.
...
```shell
revampush release-react <appName> <platform>
...
revampush release MyApp-iOS ./CodePush 1.0.0
```
...
```shell
revampush release-react MyApp-iOS ios
```
...
```shell
revampush release-react MyApp-Android android -p "./foo/bar/"
revampush release-react MyApp-Android android -p "./foo/bar/build.gradle"
```
...
```shell
revampush release-react MyApp-iOS ios -p "./foo/bar/MyFile.plist"
```
...
```shell
revampush patch <appName> <deploymentName>
[--label <releaseLabel>]
...
revampush patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
revampush patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `revampush deployment history` command and refer to the `Label` column.
...
revampush patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
revampush promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
revampush promote MyApp-iOS Staging Production -t "*"
```
...
```
revampush rollback <appName> <deploymentName>
revampush rollback MyApp-iOS Production
```
...
```
revampush rollback MyApp-iOS Production --targetRelease v34
```
...
```
revampush deployment history <appName> <deploymentName>
```
...
```
revampush deployment clear <appName> <deploymentName>
```
...
```shell
revampush release-react <appName> <platform> --privateKeyPath private.pem
```
